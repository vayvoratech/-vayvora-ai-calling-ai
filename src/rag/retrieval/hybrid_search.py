...
class BM25Retriever:
    """Lightweight BM25 keyword retriever for the Vayvora knowledge base."""
    ...
    def search(
        ...
        for index, document in enumerate(self.documents):
            ...
            for term in query_tokens:
                ...
                idf = math.log(
                    1 + (self.total_documents - document_frequency + 0.5)
                    / (document_frequency + 0.5)
                )

                denominator = (
                    frequency
                    + self.k1 * (1 - self.b + self.b * (document_length / self.average_document_length))
                )
